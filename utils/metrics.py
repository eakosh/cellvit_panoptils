...
def _get_instance_types(instance_map: np.ndarray, type_map: np.ndarray, ids: np.ndarray, 
                        exclude_labels: Tuple[int, ...] = (0,)) -> Dict[int, int]:
    types = {}
    for inst_id in ids:
...
def _weighted_class_det_f1(
    paired_gt_t: np.ndarray,
    paired_pred_t: np.ndarray,
    unpaired_pred_t: np.ndarray,
    unpaired_gt_t: np.ndarray,
    class_set: set,
    weights: tuple,
) -> Optional[float]:
    """Weighted detection F1 for a set of classes (single class or superclass group).

    Returns None when the denominator is zero (no relevant instances).
    """
    cs = list(class_set)
    if len(paired_gt_t) > 0:
        sel = np.isin(paired_gt_t, cs) | np.isin(paired_pred_t, cs)
        pt, pp = paired_gt_t[sel], paired_pred_t[sel]
        tp = int((np.isin(pt, cs) & np.isin(pp, cs)).sum())
        fp = int((~np.isin(pt, cs) & np.isin(pp, cs)).sum())
        fn = int((np.isin(pt, cs) & ~np.isin(pp, cs)).sum())
    else:
        tp = fp = fn = 0
    fp_d = int(np.isin(unpaired_pred_t, cs).sum()) if len(unpaired_pred_t) > 0 else 0
    fn_d = int(np.isin(unpaired_gt_t,   cs).sum()) if len(unpaired_gt_t)   > 0 else 0
    t = 2 * tp + weights[0] * fp + weights[1] * fn + weights[2] * fp_d + weights[3] * fn_d
    return (2 * tp / t) if t > 0 else None


def match_instances_by_centroid(pred_instances: np.ndarray, gt_instances: np.ndarray,
                                radius: float = 12.0) -> Tuple[List[Tuple[int, int]], List[int], List[int]]:
    """
    Args:
...
def match_instances(pred_instances: np.ndarray, gt_instances: np.ndarray, 
                    iou_threshold: float = 0.5) -> Tuple[List[Tuple[int, int]], List[int], List[int], List[float]]:
    """
    Args:
        pred_instances: predicted instance map [H, W]
        gt_instances: ground truth instance map [H, W]
    ...
    for i, pred_id in enumerate(pred_ids):
        ...
        for j, gt_id in enumerate(gt_ids):
            if not _bbox_overlap(pr1, pr2, pc1, pc2, g_rmin[gt_id], g_rmax[gt_id], 
                                 g_cmin[gt_id], g_cmax[gt_id]):
                continue
    ...
    if iou_threshold >= 0.5:
        # matched pairs are provably unique
...
def compute_pq(pred_instances: np.ndarray, gt_instances: np.ndarray, 
               iou_threshold: float = 0.5) -> Dict[str, float]:
    matched_pairs, unmatched_pred, unmatched_gt, matched_ious = match_instances(pred_instances, gt_instances, iou_threshold)
...
def compute_tissue_dice(pred_tissue: np.ndarray, gt_tissue: np.ndarray,
                        num_classes: int = 9, ignore_classes: set = None) -> Dict[str, float]:
    ...
    for c in range(num_classes):
        ...
        gt_c = (gt_tissue == c)
        t = pred_c.sum() + gt_c.sum()
        if t == 0:
            continue
        dice = 2.0 * (pred_c & gt_c).sum() / t
...
class MetricsAggregator:
    _W = [2, 2, 1, 1]   # [w_FPc, w_FNc, w_FPd, w_FNd] 

    def __init__(
        self,
        num_classes: int = 0,
        excluded_nuclei_classes: Optional[Iterable[int]] = None,
        centroid_radius: float = 12.0,
        superclass_map: Optional[Dict[str, List[int]]] = None,
    ):
        self.num_classes = num_classes
        self._excluded: Set[int] = set(excluded_nuclei_classes) if excluded_nuclei_classes else set()
        self.centroid_radius = centroid_radius
        self.superclass_map: Dict[str, List[int]] = superclass_map or {}
        self.reset()
    ...
    def reset(self):
        ...
        if self.num_classes > 0:
            self.per_class_pq: Dict[int, List[float]] = {c: [] for c in range(1, self.num_classes) 
                                                        if c not in self._excluded} 
            self.per_class_det: Dict[int, List[float]] = {c: [] for c in range(1, self.num_classes) 
                                                          if c not in self._excluded}
        else:
            self.per_class_pq = {} 
            self.per_class_det = {}
        ...
        self.per_superclass_det: Dict[str, List[float]] = {sc: [] for sc in self.superclass_map}

        self._auroc_probs: List[np.ndarray] = []
        self._auroc_labels: List[int] = []
    ...
    def update(
        ...
        pred_type_probs: np.ndarray = None,  
    ):
        ...
        if pred_type_map is not None and gt_type_map is not None:
            matched_pairs, unmatched_pred, unmatched_gt = match_instances_by_centroid(pred_instances, gt_instances, self.centroid_radius)

            TP_d = len(matched_pairs)
            FP_d = len(unmatched_pred)
            FN_d = len(unmatched_gt)

            det_f1 = (2 * TP_d / (2 * TP_d + FP_d + FN_d) if (2 * TP_d + FP_d + FN_d) > 0 else 0.0)
            ...
            if self.per_class_det:
                ...
                paired_pred_t = np.array([pred_types.get(p, 0) for p, _ in matched_pairs], dtype=np.int32)
                paired_gt_t = np.array([gt_types.get(g, 0) for _, g in matched_pairs], dtype=np.int32)

                unpaired_pred_t = np.array([pred_types.get(p, 0) for p in unmatched_pred], dtype=np.int32)
                unpaired_gt_t = np.array([gt_types.get(g, 0) for g in unmatched_gt], dtype=np.int32)

                for c in self.per_class_det:
                    if not np.any(gt_type_map == c):
                        continue
                    score = _weighted_class_det_f1(paired_gt_t, paired_pred_t, unpaired_pred_t, unpaired_gt_t, {c}, self._W)
                    if score is not None:
                        self.per_class_det[c].append(score)

                for sc_name, sc_classes in self.superclass_map.items():
                    sc_set = set(sc_classes)
                    if not np.any(np.isin(gt_type_map, list(sc_set))):
                        continue
                    score = _weighted_class_det_f1(paired_gt_t, paired_pred_t, unpaired_pred_t, unpaired_gt_t, sc_set, self._W)
                    if score is not None:
                        self.per_superclass_det[sc_name].append(score)

            if pred_type_probs is not None and matched_pairs:
                _ignore_arr = np.array(sorted(self._excluded), dtype=np.int64)
                C = pred_type_probs.shape[0]

                probs_flat = pred_type_probs.reshape(C, -1)
                pred_inst_flat = pred_instances.reshape(-1)
                gt_inst_flat = gt_instances.reshape(-1)
                gt_type_flat = gt_type_map.reshape(-1)

                for pred_id, gt_id in matched_pairs:
                    p_idx = np.nonzero(pred_inst_flat == pred_id)[0]
                    if p_idx.size == 0:
                        continue
                    mean_probs = probs_flat[:, p_idx].mean(axis=1).astype(np.float32)

                    g_idx = np.nonzero(gt_inst_flat == gt_id)[0]
                    if g_idx.size == 0:
                        continue
                    gt_labels = gt_type_flat[g_idx]
                    if _ignore_arr.size > 0:
                        gt_labels = gt_labels[~np.isin(gt_labels, _ignore_arr)]
                    if gt_labels.size == 0:
                        continue
                    gt_class = int(np.bincount(gt_labels.astype(np.int64)).argmax())

                    self._auroc_probs.append(mean_probs)
                    self._auroc_labels.append(gt_class)
    ...
    def compute(self) -> Dict[str, float]:
        ...
        if self.per_superclass_det:
            sc_f1s = []
            for sc_name, scores in self.per_superclass_det.items():
                if scores:
                    avg = float(np.mean(scores))
                    metrics[f'sc_f1_{sc_name}'] = avg
                    sc_f1s.append(avg)
            if sc_f1s:
                metrics['sc_f1_avg'] = float(np.mean(sc_f1s))

        if self.superclass_map and self.per_class_pq:
            for sc_name, sc_classes in self.superclass_map.items():
                sc_pqs = []
                for c in sc_classes:
                    if c in self.per_class_pq and self.per_class_pq[c]:
                        sc_pqs.append(float(np.mean(self.per_class_pq[c])))
                if sc_pqs:
                    metrics[f'sc_pq_{sc_name}'] = float(np.mean(sc_pqs))

        if len(self._auroc_probs) >= 10:
            try:
                from sklearn.metrics import roc_auc_score

                all_probs = np.stack(self._auroc_probs, axis=0) 
                all_labels = np.array(self._auroc_labels, dtype=np.int64)

                active_classes = [c for c in range(1, self.num_classes) if c not in self._excluded]
                ...
                for c in active_classes:
                    ...
                    if pos == 0 or pos == len(y_true):
                        continue
                    if c >= all_probs.shape[1]:
                        continue
                    try:
                        auc = float(roc_auc_score(y_true, all_probs[:, c]))
                        metrics[f'auroc_class_{c}'] = auc
                        per_class_aurocs.append(auc)
                    except Exception:
                        pass
                if per_class_aurocs:
                    metrics['auroc_macro'] = float(np.mean(per_class_aurocs))

                for sc_name, sc_classes in self.superclass_map.items():
                    sc_list = [c for c in sc_classes if c < all_probs.shape[1]]
                    if not sc_list:
                        continue
                    y_true = np.isin(all_labels, sc_list).astype(np.int32)
                    pos = int(y_true.sum())
                    if pos == 0 or pos == len(y_true):
                        continue
                    y_score = all_probs[:, sc_list].sum(axis=1)
                    try:
                        auc = float(roc_auc_score(y_true, y_score))
                        metrics[f'sc_auroc_{sc_name}'] = auc
                    except Exception:
                        pass

                sc_aurocs = [v for k, v in metrics.items() if k.startswith('sc_auroc_')]
                if sc_aurocs:
                    metrics['sc_auroc_avg'] = float(np.mean(sc_aurocs))
            except ImportError:
                pass
    ...
    def __repr__(self):
        m = self.compute()
        parts = [f"bPQ={m['pq']:.4f}  SQ={m['sq']:.4f}  DQ={m['rq']:.4f}"]

        if 'pq_class_avg' in m:
            parts.append(f"mPQ={m['pq_class_avg']:.4f}")
        if 'det_f1' in m:
            parts.append(f"F1_d={m['det_f1']:.4f}  P_d={m['det_precision']:.4f}  R_d={m['det_recall']:.4f}")
        if 'cls_f1_avg' in m:
        ...
        sc_parts = []
        for sc in self.superclass_map:
            f1_key = f"sc_f1_{sc}"
            pq_key = f"sc_pq_{sc}"
            auc_key = f"sc_auroc_{sc}"
            if f1_key in m or pq_key in m or auc_key in m:
                f1_s = f"F1={m[f1_key]:.3f}" if f1_key in m else ""
                pq_s = f"PQ={m[pq_key]:.3f}" if pq_key in m else ""
                auc_s = f"AUC={m[auc_key]:.3f}" if auc_key in m else ""
                sc_parts.append(f"{sc}[{'/'.join(filter(None, [f1_s, pq_s, auc_s]))}]")

        if sc_parts:
            parts.append("SC: " + "  ".join(sc_parts))
        if 'auroc_macro' in m:
            parts.append(f"AUROC_macro={m['auroc_macro']:.4f}")
        return "  |  ".join(parts)
